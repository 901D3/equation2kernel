const $windowFunc2D = (function () {
  ...
  const _getWindowFunctionLUT = (width, height, equation) => {
    ...
    const key = width + "x" + height + " | " + trimmed;

    if (!windowFuncLUT.has(key)) {
      const array = new Float64Array(sqSz).fill(1);
      ...
      for (let y = 0; y < height; y++) {
        ...
        for (let x = 0; x < width; x++) {
          array[yOffs + x] *= cp(x, y, width, height);
        }
      }
